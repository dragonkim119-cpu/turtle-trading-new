...
| `apps/engine` | 24h 워커: 봉 마감 판정, 손절 1분 감시, 1분 변동성 가드, RSS 10분 수집, 매크로 6h 수집, 텔레그램 |
...
- **타임스톱** (`timeStop.bars`, 기본 off): 진입 후 N봉 내 +1R 미도달 시 청산 권고. 교차검증 결과 약세·횡보장 개선하나 강한 추세장에선 느린 승자 절단으로 PF↓ (6개 중 3개만 유지/개선) → 게이트 미통과, 기본 off·심볼별 opt-in. backtest·실시간 엔진 양쪽 지원
- **리스크**: 거래당 2%. 수량 = 자산×리스크% ÷ (2×ATR)
- **보완 필터** (개별 on/off, 진입에만 적용·청산엔 미적용): ADX(14)≥20, 거래량≥1.5×평균, Rolling VWAP(30일) 방향, 펀딩비 ±0.1% 과열 차단, **OI 확인**(24h 미결제약정 증가 = 신규 자금·진짜 돌파, 기본 off)
- **OI 필터 주의**: 펀딩·OI는 무료 API로 과거 ~30일만 조회돼 **장기 백테스트 불가** → backtest는 강제 off, 실시간 엔진 전용·기본 off·심볼별 opt-in. `openInterestHist` 1h×24개로 24h 변화율 계산
...
**매크로 지표** (표시 전용, 신호 게이트 미사용): 엔진이 6시간마다 FRED 공개 CSV로 DXY(DTWEXBGS)·VIX(VIXCLS)·미10Y(DGS10) 수집 → `macro_snapshots` 테이블 → 뉴스탭 카드 스파크라인. Stooq는 봇 차단 생겨 FRED로 대체. 매크로를 신호 게이트화하려면 백테스트 검증 선행 필요.
...
5. **필터 AND→스코어링** — 지금 필터 다중 on 시 암묵적 AND(거래수 급감). 점수제(N개 중 M개 통과)로 표본 유지하며 저품질 신호만 제외

주의: 스윕 상위 결과(PF 1.37 등)는 단일기간·단일심볼 — crossval 안 거치면 노이즈일 확률 높음. 위 항목도 반드시 crossval 게이트 통과 후 채택.
